...
class Telegram(object):
	...
	def poll(self):
		...
		try:
			c = bytes(self._proc.stdout.read(1), 'utf-8')
		except:
			c = b''
		if not c:
			c = b''
	...
	def msg(self, peer, message):
		self.tgin.write(bytes(' '.join(['msg', peer, message]) + '\n', 'utf-8'))
		self.tgin.flush()

	def send_audio(self, peer, path):
		self.tgin.write(bytes(' '.join(['send_audio', peer, path]) + '\n', 'utf-8'))
		self.tgin.flush()

	def send_contact(self, peer, phone, first_name, last_name):
		self.tgin.write(bytes(' '.join(['send_contact', peer, phone, first_name, last_name]) + '\n', 'utf-8'))
		self.tgin.flush()

	def send_document(self, peer, path):
		self.tgin.write(bytes(' '.join(['send_document', peer, path]) + '\n', 'utf-8'))
		self.tgin.flush()

	def send_photo(self, peer, path):
		self.tgin.write(bytes(' '.join(['send_photo', peer, path]) + '\n', 'utf-8'))
		self.tgin.flush()

	def send_video(self, peer, path):
		self.tgin.write(bytes(' '.join(['send_video', peer, path]) + '\n', 'utf-8'))
		self.tgin.flush()

	def send_location(self, peer, latitude, longitude):
		self.tgin.write(bytes(' '.join(['send_location', peer, latitude, longitude]) + '\n', 'utf-8'))
		self.tgin.flush()

	def send_text(self, peer, path):
		self.tgin.write(bytes(' '.join(['send_text', peer, path]) + '\n', 'utf-8'))
		self.tgin.flush()
	...
	def chat_info(self, chat):
		self.tgin.write(bytes(''.join(['chat_info ', chat, '\n']), 'utf-8'))
		self.tgin.flush()
	...
	def chat_add_user(self, chat, user, msgs_to_forward="100"):
		...
		"""
		self.tgin.write(bytes(' '.join(['chat_add_user', chat, user,msgs_to_forward]) + '\n', 'utf-8'))
		self.tgin.flush()
	...
	def chat_del_user(self, chat, user):
		...
		"""
		self.tgin.write(bytes(' '.join(['chat_del_user', chat, user]) + '\n', 'utf-8'))
		self.tgin.flush()
...
# create_group_chat <user> <chat topic> - creates a groupchat with user, use chat_add_user to add more users
	def create_group_chat(self,user,chat_topic):
		...
		self.tgin.write(bytes(' '.join(['create_group_chat', user, chat_topic]) + '\n', 'utf-8')) #TODO: Can chat_topic contain spaces, does it need quotes?
		self.tgin.flush()

	def user_info(self, user):
		self.tgin.write(bytes(''.join(['user_info ', user, '\n']), 'utf-8'))
		self.tgin.flush()

	def mark_read(self, peer):
		self.tgin.write(bytes(''.join(['mark_read ', peer, '\n']), 'utf-8'))
		self.tgin.flush()

	def chat_with_peer(self, peer):
		self.tgin.write(bytes(''.join(['chat_with_peer ', peer, '\n']), 'utf-8'))
		self.tgin.flush()
	...
	def del_contact(self, user):
		self.tgin.write(bytes(''.join(['del_contact ', user, '\n']), 'utf-8'))
		self.tgin.flush()

	def fwd(self, peer, msg_id):
		self.tgin.write(bytes(' '.join(['fwd', peer, msg_id,'\n']), 'utf-8'))
		self.tgin.flush()

	def fwd_media(self, peer, msg_id):
		self.tgin.write(bytes(' '.join(['fwd_media', peer, msg_id,'\n']), 'utf-8'))
		self.tgin.flush()

	def raw_input(self, raw):
		self.tgin.write(bytes(''.join([raw,'\n']), 'utf-8'))
		self.tgin.flush()
	...
	def chat_set_photo(self, chat, path):
		...
		"""
		self.tgin.write(bytes(' '.join(['chat_set_photo', chat, path]) + '\n', 'utf-8'))
		self.tgin.flush()
	...
	def set_profile_photo(self, path):
		...
		"""
		self.tgin.write(bytes(' '.join(['set_profile_photo', path]) + '\n', 'utf-8'))
		self.tgin.flush()
	...
	def import_card(self,card):
		...
		"""
		self.tgin.write(bytes(' '.join(['import_card', card]) + '\n', 'utf-8'))
		self.tgin.flush()
	...
	def load_photo(self, msg_id):
		...
		"""
		self.tgin.write(bytes(' '.join(['load_photo', msg_id,'\n']), 'utf-8'))
		self.tgin.flush()
	...
	def load_video(self, msg_id):
		...
		Downloads file to downloads dirs.
		"""
		self.tgin.write(bytes(' '.join(['load_video', msg_id,'\n']), 'utf-8'))
		self.tgin.flush()
	...
	def load_audio(self, msg_id):
		...
		Downloads file to downloads dirs.
		"""
		self.tgin.write(bytes(' '.join(['load_audio', msg_id,'\n']), 'utf-8'))
		self.tgin.flush()
	...
	def load_document(self, msg_id):
		...
		Downloads file to downloads dirs.
		"""
		self.tgin.write(bytes(' '.join(['load_document', msg_id,'\n']), 'utf-8'))
		self.tgin.flush()
	...
	def load_video_thumb(self, msg_id):
		...
		Downloads file to downloads dirs.
		"""
		self.tgin.write(bytes(' '.join(['load_video_thumb', msg_id,'\n']), 'utf-8'))
		self.tgin.flush()
	...
	def load_document_thumb(self, msg_id):
		...
		Downloads file to downloads dirs.
		"""
		self.tgin.write(bytes(' '.join(['load_document_thumb', msg_id,'\n']), 'utf-8'))
		self.tgin.flush()
