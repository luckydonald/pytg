...
print_message = re.compile('^(?P<msgid>\d+)\ \[(?P<timestamp>.{5,6})\]\ \[?(?P<media>.+)?\]?\ ?(?P<geolink>https\:\/\/map\.google\.com\/\?=[\d\.\,])?\ \{print_message}\ (chati?d?\#(?P<chat>.+)\ )?useri?d?\#(?P<user>.+)\ (?P<dir>[<>«»]{3})\ (?P<message>.*)\{end_print_message\}', re.U|re.M|re.S)
