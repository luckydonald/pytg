...
print_message_data = re.compile('^(?P<msgid>\d+)\ \[(?P<timestamp>.{5,6})\]\ (?:\[(?P<media>.+)\])?\ \{print_message}\ (chati?d?\#(?P<chat>.+)\#(?P<chatid>\d+)\ )?useri?d?\#(?P<user>.+)\#(?P<userid>\d+)\ (?P<dir>[<>«»]{3})\ (?P<message>.*)\{end_print_message\}', re.U|re.M|re.S)
